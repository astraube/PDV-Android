...
public class TableRowTextView extends TableRow {
	...
	public TableRowTextView(Context context, String text1, String text2, int sizeSpacer) {
        ...
        textView1.setTextColor(ResourcesCompat.getColor(getContext().getResources(), R.color.gray, null));
		...
		if (Build.VERSION.SDK_INT < 23) {
			textView2.setTextAppearance(context, R.style.textProductMedium);
		} else {
			textView2.setTextAppearance(R.style.textProductMedium);
		}
		textView2.setTextSize(context.getResources().getDimensionPixelSize(R.dimen.text_size_big));
        textView2.setTextColor(ResourcesCompat.getColor(getContext().getResources(), R.color.gray, null));

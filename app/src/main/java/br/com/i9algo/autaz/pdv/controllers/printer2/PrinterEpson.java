...
public class PrinterEpson {
	...
	public static int PRINTER_LANG = Printer.MODEL_ANK;

	private static PrinterEpson _instance = null;
	...
	public static PrinterEpson getInstance(EpsonReceiveListener owner) {
		if (_instance == null)
			_instance = new PrinterEpson(owner);
		
		return PrinterEpson._instance;
	}

	public static void getSelectSeries(final Activity activity) {
		if (PRINTER_SERIES == -1) {

			new MaterialDialog.Builder(activity)
					.iconRes(R.drawable.ic_print_black_48dp)
					.title(R.string.printer_series_select)
					.items(R.array.arr_printer_series)
					.negativeText(R.string.action_cancel)
					.cancelable(false)
					.itemsCallback(new MaterialDialog.ListCallback() {
						@Override
						public void onSelection(MaterialDialog dialog, View view, int which, CharSequence text) {
							SimpleToast.info(activity, activity.getString(R.string.printer_series_selected, text.toString()));
							PRINTER_SERIES = which;
						}
					})
					.show();
		}
	...
	private boolean createReceiptDataSale(final Cupom cupom) {
		...
		try {
			...
			if (cupom.getMethod() != null) {
				...
				if (m != null) {
					textData.append(this.mOwner.getActivity().getString(R.string.divider));
					textData.append(this.mOwner.getActivity().getString(R.string.txt_payment_with, m.toLowerCase()) + "\n");
					mPrinter.addText(textData.toString());
					textData.delete(0, textData.length());
				}
			}
			...
			textData.append(this.mOwner.getActivity().getString(R.string.divider));
			textData.append(this.mOwner.getActivity().getString(R.string.cupom_not_fiscal));
			textData.append(this.mOwner.getActivity().getString(R.string.divider));
			mPrinter.addText(textData.toString());
			textData.delete(0, textData.length());
			mPrinter.addFeedLine(1);
	...
	/*private boolean createReceiptDataOriginal() {
		String method = "";
		Bitmap logoData = BitmapFactory.decodeResource(this.mOwner.getActivity().getResources(), R.drawable.ic_launcher);
		StringBuilder textData = new StringBuilder();
		final int barcodeWidth = 2;
